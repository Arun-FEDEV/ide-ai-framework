# Context Engineering Compaction Prompt

Compact the session into an operational handoff that preserves the workflow state.
...
11. Verification evidence: exact command, result, important output summary, and automated versus manual verification status.
...
- Repeated search output after extracting file/symbol facts.
- API payloads, browser dumps, telemetry dumps, and huge JSON blobs after extracting relevant fields.
...
If you cannot confidently preserve the goal, Shared-Picture Contract, current phase, artifact paths, verification evidence, and next action, stop and ask the user rather than continuing from a weak compacted state.
