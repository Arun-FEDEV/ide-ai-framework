...
- Read-only first. Present recommendations before applying labels, comments, closes, or issue edits.
- Require explicit approval immediately before every GitHub mutation command.
- Stay on main `gpt-5.5/xhigh`; do not spawn subagents unless the user explicitly asks.
- If a workflow says "do it" after a recommendation, restate the exact `gh` mutation command(s) that will run before running them.
