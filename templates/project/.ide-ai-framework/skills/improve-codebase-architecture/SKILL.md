...
- Stay inline on the main `gpt-5.5/xhigh` thread unless the user explicitly asks for subagents, delegation, or parallel exploration.
- If the upstream workflow says to use an Agent tool for exploration, do the exploration locally with `rg`, targeted file reads, and repo tests instead.
- Do not write `CONTEXT.md` or ADR changes until the user has selected a candidate and confirmed the terminology or decision being recorded.
...
Then walk the codebase with targeted local exploration. Don't follow rigid heuristics — explore organically and note where you experience friction:
