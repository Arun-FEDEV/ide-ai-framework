...
- Draft issues first and ask for explicit approval before running `gh issue create`.
- Do not modify or close parent issues unless the user explicitly asks.
- Keep issue bodies durable: behavior, acceptance criteria, and dependencies over file paths or implementation trivia.
...
### 5. Draft and create the GitHub issues

For each approved slice, draft a GitHub issue using the issue body template below. Show the titles and bodies to the user. Only after explicit approval, create them using `gh issue create`.
